...
def doVerifyOtp(phone,token,data):
    ...
    if "status" in response1["data"] and response1["data"]["status"] == 2 and "suspended" in response1["data"] and not response1["data"]["suspended"]:
            ...
            os.remove(requestFilePath)
            return jsonify(
                {"status":"Logged in successfully","InstallationId":response1['data']['installationId'],"response":response1}
            )
...
def searchFunction(phoneNumber,args_name,args_email,args_raw):
    ...
    try:
        ...
        search_result = asyncio.run(search_phonenumber(
            phoneNumber, country_code, installation_id))

        if args_name and not args_email:
            try:
...
def loginCheck(phoneNumber):
    if not check_for_file():
        return jsonify({"status":"error","errorMsg":"Do Login."})
    try:
        with open(authKeyFilePath, "r") as auth_key_file:
            data = auth_key_file.read()
            json_auth_key = json.loads(data)
            country_code = json_auth_key["phones"][0]["countryCode"]
            installation_id = json_auth_key["installationId"]
            phone_Number=json_auth_key["phones"][0]["phoneNumber"]

            print("Print="+str(phone_Number)+"#2#"+phoneNumber+"#")
        if str(phone_Number)==str(phoneNumber):    
            return jsonify({"status":"LoggedIn","cCode":str(country_code), "installationId":installation_id, "phoneNumber":str(phone_Number)})
        else :
            return jsonify({"status":"error","errorMsg":"Not LoggedIn"})
